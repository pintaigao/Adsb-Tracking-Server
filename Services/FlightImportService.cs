...
public sealed class FlightImportService(HttpClient httpClient, IOptions<FlightTrainingServerOptions> options, IConfiguration configuration) {
	 ...
	 */
	public async Task ImportScheduledTrackAsync(WatchSchedule schedule, WatchExecution execution, CancellationToken cancellationToken) {
		/*
		 * 允许优先走环境变量，其次走 Options 配置。
		 * 如果没有配置主后端地址，就直接跳过回调。
		 */
		var baseUrl = ResolveValue(configuration["FLIGHT_TRAINING_SERVER_BASE_URL"], options.Value.BaseUrl);
		 ...
		 */
		var serviceToken = ResolveValue(configuration["FLIGHT_TRAINING_SERVER_SERVICE_TOKEN"], options.Value.ServiceToken);
		...
		throw new InvalidOperationException($"Flight import callback failed ({(int)response.StatusCode}): {raw}");
	}
